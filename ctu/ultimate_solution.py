...
class MetricsCallback(TrainerCallback):
    """Callback to capture and save training metrics."""
    def on_log(self, args, state, control, logs=None, **kwargs):
        """Capture metrics from training logs."""
        if logs is None:
            return
        ...
        # Training metrics
        if 'loss' in logs:
            val = safe_float(logs['loss'])
            if val is not None:
                register_plot_point('Training Loss', val)
        if 'grad_norm' in logs:
            val = safe_float(logs['grad_norm'])
            if val is not None:
                register_plot_point('Gradient Norm', val)
        if 'learning_rate' in logs:
            val = safe_float(logs['learning_rate'])
            if val is not None:
                register_plot_point('Learning Rate', val)
        
        # Evaluation metrics
        if 'eval_loss' in logs:
            val = safe_float(logs['eval_loss'])
            if val is not None:
                register_plot_point('Validation Loss', val)
        if 'eval_mse' in logs:
            val = safe_float(logs['eval_mse'])
            if val is not None:
                register_plot_point('Validation MSE', val)
        if 'eval_mae' in logs:
            val = safe_float(logs['eval_mae'])
            if val is not None:
                register_plot_point('Validation MAE', val)
        if 'eval_rmse' in logs:
            val = safe_float(logs['eval_rmse'])
            if val is not None:
                register_plot_point('Validation RMSE', val)
        if 'eval_spearman' in logs:
            val = safe_float(logs['eval_spearman'])
            if val is not None:
                register_plot_point('Validation Spearman', val)
...
def create_all_plots():
    ...
    if not plot_data:
        print("No plot data to display.")
        return
    
    # Create output directory for plots
    os.makedirs(os.path.join(PREDICTIONS_ROOT, "plots"), exist_ok=True)
    
    # Create a separate figure for each plot
    for plot_name, values in plot_data.items():
        # Create a new figure for each plot (separate window)
        fig, ax = plt.subplots(figsize=(10, 6))
        
        # Plot the data
        x_values = range(1, len(values) + 1)
        ax.plot(x_values, values, marker='o', linestyle='-', linewidth=2, markersize=4)
        ...
        if len(values) > 0:
            ...
            ax.axhline(y=mean_val, color='r', linestyle='--', alpha=0.5, label=f'Mean: {mean_val:.4f}')
            ax.axhline(y=min_val, color='g', linestyle=':', alpha=0.5, label=f'Min: {min_val:.4f}')
            ax.axhline(y=max_val, color='b', linestyle=':', alpha=0.5, label=f'Max: {max_val:.4f}')
            ax.legend()
        
        # Save the plot to a separate file
        filename = os.path.join(PREDICTIONS_ROOT, "plots", f"{plot_name.replace(' ', '_').replace('/', '_')}.png")
    ...
    print(f"\nCreated {len(plot_data)} separate plots.")
...
def build_enhanced_text(sample: dict, clean_format: bool = True) -> str:
    """Build text with all relevant context.
    
    Args:
        sample: Sample dictionary with homonym, meaning, and narrative parts
        clean_format: If True, use natural text without brackets/colons. 
                     If False, use structured format with brackets.
    """
    parts = []
    
    # Critical: Include homonym and meaning for disambiguation
    if sample.get("homonym") and sample.get("judged_meaning"):
        if clean_format:
            # Natural format: "The word 'bank' means financial institution"
            parts.append(f"The word '{sample['homonym']}' means {sample['judged_meaning']}")
        else:
            # Structured format: "[Homonym: bank] [Meaning: financial institution]"
            parts.append(f"[Homonym: {sample['homonym']}]")
            parts.append(f"[Meaning: {sample['judged_meaning']}]")
    elif sample.get("homonym"):
        if clean_format:
            parts.append(f"The word is '{sample['homonym']}'")
        else:
            parts.append(f"[Homonym: {sample['homonym']}]")
    elif sample.get("judged_meaning"):
        if clean_format:
            parts.append(f"The meaning is {sample['judged_meaning']}")
        else:
            parts.append(f"[Meaning: {sample['judged_meaning']}]")
...
def prepare_dataset(split_data: Dict[str, dict], clean_format: bool = True):
    ...
    for k, v in split_data.items():
        ids.append(k)
        texts.append(build_enhanced_text(v, clean_format=clean_format))
...
def main():
    ...
    if use_clean_format:
        ...
        print("Using structured format with brackets")
    ...
    base_training_args = {
        "output_dir": os.path.join(PREDICTIONS_ROOT, "ultimate_checkpoints"),
        ...
        "logging_dir": os.path.join(PREDICTIONS_ROOT, "ultimate_logs"),
        "logging_steps": 50,
        "save_strategy": "epoch",
        "save_total_limit": 3,
        "seed": args.seed,
    ...
    if use_eval_strategy:
        try:
            ...
            training_args.metric_for_best_model = "spearman"
    ...
    print(f"Validation MSE: {float(val_results.get('eval_mse', 0)):.4f}")
    print(f"Validation MAE: {float(val_results.get('eval_mae', 0)):.4f}")
    print(f"Validation RMSE: {float(val_results.get('eval_rmse', 0)):.4f}")
    print(f"Validation Spearman: {float(val_results.get('eval_spearman', 0)):.4f} (p={float(val_results.get('eval_spearman_p', 0)):.4f})")
    print(f"Validation Accuracy: {float(val_results.get('eval_accuracy', 0)):.4f}")
    ...
    train_pred_classes = np.sum(train_probs > 0.5, axis=1) + 1
    train_pred_classes = np.clip(train_pred_classes, 1, 5).astype(int)
    ...
    with open(args.train_output, "w", encoding="utf-8") as f:
        for i, p in zip(train_ids, train_pred_classes.tolist()):
            f.write(json.dumps({"id": i, "label": int(p)}) + "\n")

    print(f"\nWrote training predictions to {args.train_output}")
    print(f"\nTrain prediction distribution:")
    print(f"  Min: {train_pred_classes.min()}, Max: {train_pred_classes.max()}, Mean: {train_pred_classes.mean():.2f}")
    unique, counts = np.unique(train_pred_classes, return_counts=True)
    for u, c in zip(unique, counts):
        print(f"  Class {int(u)}: {c} predictions ({100*c/len(train_pred_classes):.1f}%)")
